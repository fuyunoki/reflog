...
export type DomainError =
  ...
  | { readonly type: 'TagAlreadyExists'; readonly tag: string }
  | { readonly type: 'TagNotFound'; readonly tag: string }
  | { readonly type: 'DetachedHeadNotAllowed' }
  | { readonly type: 'NothingToCommit' }
  | { readonly type: 'AlreadyUpToDate'; readonly branch: string }
  | { readonly type: 'AlreadyApplied'; readonly commitId: string }
  ...
  | { readonly type: 'CannotRebaseMerge'; readonly commitId: string }
...
export const describe = (e: DomainError): string => {
  switch (e.type) {
    ...
    case 'TagAlreadyExists':
      return `tag already exists: ${e.tag}`;
    case 'TagNotFound':
      return `tag not found: ${e.tag}`;
